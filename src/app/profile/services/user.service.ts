import { Injectable } from '@angular/core';
import { Router } from '@angular/router';
import { BehaviorSubject, Subject } from 'rxjs';
import { AuthService } from 'src/app/auth/auth.service';
import {
  UserProfile,
  UserAccount,
  AuthResponse,
  LoggedInUserSession,
} from '../models/User.model';

@Injectable({
  providedIn: 'root',
})
export class UserService {
  usersMap: Map<string, UserProfile> = new Map();
  private _loggedInUser: UserProfile;
  loggedInUserChanged = new BehaviorSubject<UserProfile>(null);
  loggedInUserSessionChanged = new BehaviorSubject<LoggedInUserSession>(null);
  errorHappened = new Subject<string>();
  private _newlyCreatedUserId: string;
  private _logoutTimer: any;

  public get loggedInUser() {
    return this._loggedInUser;
  }

  private set loggedInUser(userToBeLoggedIn: UserProfile) {
    this._loggedInUser = userToBeLoggedIn;
    this.loggedInUserChanged.next(this._loggedInUser);
  }

  constructor(private _authService: AuthService, private _router: Router) {
    let image =
      'data:image/jpeg;base64,';
    let image2 =
      'https://m.media-amazon.com/images/M/MV5BZDk1ZmU0NGYtMzQ2Yi00N2NjLTkyNWEtZWE2NTU4NTJiZGUzXkEyXkFqcGdeQXVyMTExNDQ2MTI@._V1_.jpg';
    let ahmedUser: UserProfile = new UserProfile(
      // '',
      'midooo',
      'ahmed',
      image,
      image,
      new Date(2004, 11, 12),
      new Date(),
      ['khan'],
      [],
      'Egypt',
      'this is description',
      'https://www.youtube.com/watch?v=NoAqRpHSNM8'
    );

    let khan: UserProfile = new UserProfile(
      // '',
      'khan',
      'khankhoun',
      image2,
      image2,
      new Date(2011, 11, 12),
      new Date(),
      [],
      ['midooo'],
      'Egypt',
      'this is description'
    );

    // this.usersMap.set(ahmedUser.username, ahmedUser);
    // this.usersMap.set(khan.username, khan);

    // this.loggedInUser = this.usersMap.get('khan')!;
  }

  createAccount(userAccount: UserAccount) {
    this._authService.createAccount(userAccount).subscribe((response) => {
      this._newlyCreatedUserId = response.localId;

      this.modifyUserSession(
        response.email,
        response.localId,
        response.idToken,
        +response.expiresIn
      );
    });
  }

  createProfile(
    username: string,
    name: string,
    birthdate: Date,
    userAccount: UserAccount
  ) {
    let newUser = new UserProfile(
      username,
      name,
      'https://via.placeholder.com/300x300.png?text=profile+image',
      'https://via.placeholder.com/300x100.png?text=cover+image',
      birthdate,
      new Date(),
      [],
      []
    );

    this._authService
      .createProfile(this._newlyCreatedUserId, newUser)
      .subscribe((response) => {
        this.loginUser(userAccount);
      });
  }

  modifyUserSession(
    email: string,
    id: string,
    token: string,
    tokenExpireation: number
  ) {
    const _loggedInUserSession = new LoggedInUserSession(
      email,
      id,
      token,
      new Date(new Date().getTime() + tokenExpireation * 1000)
    );

    this.loggedInUserSessionChanged.next(_loggedInUserSession);

    return _loggedInUserSession;
  }

  loginUser(userAccount: UserAccount) {
    this._authService.login(userAccount).subscribe({
      next: (userSessionResponse) => {
        const _loggedInUserSession = this.modifyUserSession(
          userSessionResponse.email,
          userSessionResponse.localId,
          userSessionResponse.idToken,
          +userSessionResponse.expiresIn
        );

        this._authService
          .getUserById(_loggedInUserSession.id)
          .subscribe((loggedInUserResponse) => {
            this.loggedInUser = loggedInUserResponse;

            this._router.navigate(['/home']);

            this.usersMap.set(this.loggedInUser.username, this.loggedInUser);

            this.saveUserDataOnLocalStorage(
              _loggedInUserSession,
              loggedInUserResponse
            );
          });
      },
      error: (errorMessage) => {
        this.errorHappened.next(errorMessage);
      },
    });
  }

  autoLogin() {
    let loadedUser = this.getUserDataFromLocalStorage();

    if (loadedUser) {
      const userSession = this.modifyUserSession(
        loadedUser.user.email,
        loadedUser.user.id,
        loadedUser.user._token,
        loadedUser.user._tokenExpiraitionDate
      );

      const userProfile = new UserProfile(
        loadedUser.userProfile.username,
        loadedUser.userProfile.name,
        loadedUser.userProfile.profileImage,
        loadedUser.userProfile.headerImage,
        loadedUser.userProfile.birthdate,
        loadedUser.userProfile.accountCreation,
        loadedUser.userProfile.followers,
        loadedUser.userProfile.following,
        loadedUser.userProfile.description,
        loadedUser.userProfile.location,
        loadedUser.userProfile.website
      );

      if (userSession.token) {
        this.loggedInUser = userProfile;

        this.usersMap.set(this.loggedInUser.username, this.loggedInUser);

        const tokenExpiration =
          new Date(loadedUser.user._tokenExpiraitionDate).getTime() -
          new Date().getTime();
        this.autoLogout(tokenExpiration);
      }
    }
  }

  logout() {
    this.loggedInUserChanged.next(null);
    this.loggedInUserSessionChanged.next(null);
    localStorage.removeItem('loggedInUserData');
    this._router.navigate(['explore']);

    if (this._logoutTimer) {
      clearTimeout(this._logoutTimer);
    }
  }

  autoLogout(timer: number) {
    this._logoutTimer = setTimeout(() => {
      this.logout();
    }, timer);
  }

  // to save user data on local storage
  private saveUserDataOnLocalStorage(
    user: LoggedInUserSession,
    userProfile: UserProfile
  ) {
    localStorage.setItem(
      'loggedInUserData',
      JSON.stringify({ user, userProfile })
    );
  }

  // to get user data from local storage
  private getUserDataFromLocalStorage() {
    const userData = localStorage.getItem('loggedInUserData');

    if (!userData) {
      return;
    }

    return JSON.parse(userData);
  }

  isEmailFound(email: string) {
    return this._authService.isEmailUsed(email);
  }

  getUserByUsername(username: string): UserProfile | undefined {
    if (this.isUserNameFound(username)) {
      return this.usersMap.get(username);
    }
    return undefined;
  }

  isUserNameFound(username: string): boolean {
    return this.usersMap.has(username);
  }

  followUser(userWantToFollow: UserProfile, userToBeFollowed: UserProfile) {
    if (this.findUserInFollowing(userWantToFollow, userToBeFollowed) === -1) {
      userWantToFollow.following.push(userToBeFollowed.username);
      userToBeFollowed.followers.push(userWantToFollow.username);
    }
  }

  unFollowUser(
    userWantToUnfollow: UserProfile,
    userToBeUnfollowed: UserProfile
  ) {
    if (
      this.findUserInFollowing(userWantToUnfollow, userToBeUnfollowed) !== -1
    ) {
      let indexOfUnfollowingUser = this.findUserInFollowing(
        userWantToUnfollow,
        userToBeUnfollowed
      );
      userWantToUnfollow.following.splice(indexOfUnfollowingUser, 1);

      let indexOfUnfollowedUser = this.findUserInFollowers(
        userToBeUnfollowed,
        userWantToUnfollow
      );
      userToBeUnfollowed.followers.splice(indexOfUnfollowedUser, 1);
    }
  }

  /**
   * to Check is user followed or not
   * @returns user index if found || -1 if user not found
   */
  findUserInFollowers(
    userWhoWantToKnow: UserProfile,
    userToBeSearchedFor: UserProfile
  ) {
    return userWhoWantToKnow.followers.indexOf(userToBeSearchedFor.username);
  }

  findUserInFollowing(
    userWhoWantToKnow: UserProfile,
    userToBeSearchedFor: UserProfile
  ) {
    return userWhoWantToKnow.following.indexOf(userToBeSearchedFor.username);
  }
}
